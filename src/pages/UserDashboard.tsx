            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <section className="lg:col-span-4 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 self-start">
                        <h2 className="text-lg font-bold text-slate-900 mb-2">Your Subscriptions</h2>
                        <p className="text-sm text-slate-500 mb-6">Receive alerts for your interests:</p>
                        <div className="space-y-4 mb-8">
                            {categories.map(cat => (
                                <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                    ...
                                    <span className="text-sm font-medium text-slate-700 group-hover:text-indigo-600 transition-colors">{cat}</span>
                                </label>
                            ))}
                        </div>
                        <button 
                        ...
                        <div className="space-y-4">
                            {notifications.length === 0 ? (
                                <div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
                                    ...
                                    <p className="text-slate-500 font-medium">No updates yet. Subscribe to start receiving alerts!</p>
                                </div>
                            ) : (
                                notifications.map(n => (
                                    <div 
                                        key={n._id} 
                                        className={`group bg-white p-6 rounded-2xl border transition-all hover:shadow-md ${user && n.readBy.includes(user._id) ? 'border-slate-100 opacity-75' : 'border-indigo-100 bg-indigo-50/30'}`}
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">{n.type}</span>
